	...
	$(".dropdown").each(
			...
			function() {
				if ($(this).find(".dropdown-toggle-hidden").length === 0) {
					$(this).append(
							'<a class="dropdown-toggle-hidden" data-toggle="dropdown" data-target="#'
									+ $(this).attr("id")
...
function showPage(page) {
	...
	if (selected.length > 0) {
		...
		$("#mainNav").find("a[href='" + window.location.hash + "']").closest(
				"li").addClass("active");
	} else {
		$("[data-role='page']").hide();
		...
		selected.show();
		$("#mainNav").find("li").removeClass("active");
		$("#mainNav").find("a[href='#" + selected.attr("id") + "']").closest(
				"li").addClass("active");
